...
using POS.Application.Features.Suppliers.Commands;
using POS.Application.Features.Suppliers.Queries;
...
public class SuppliersController : ControllerBase
{
    ...
    {
        var command = new CreateSupplierCommand { Dto = dto };
        var result = await _mediator.Send(command);
        if (result.IsSuccess) return Created($"/api/suppliers/{result.Value}", result.Value);
        return BadRequest(result.Error);
    }
    ...
    {
        var command = new UpdateSupplierCommand { Id = id, Dto = dto };
        var result = await _mediator.Send(command);
        if (result.IsSuccess)
